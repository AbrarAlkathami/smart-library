...
@router.get("/books/mostTrending", response_model=List[BookSchema], tags=["Books"])
def get_most_trending_books_route(db: Session = Depends(get_db)):
    return get_books_by_most_trending(db)

@router.get("/books/recentlyAdded", response_model=List[BookSchema], tags=["Books"])
def get_most_recently_added_books_route(db: Session = Depends(get_db)):
    return get_books_by_most_recently_added(db)

@router.get("/books/recentPublishedYear", response_model=List[BookSchema], tags=["Books"], operation_id="get_most_recent_published_year_books")
...
@router.get("/books/mostrecentpublishedyear", response_model=List[BookSchema], tags=["Books"], operation_id="get_most_recent_published_year_books")
def get_most_recent_published_year_books_route(db: Session = Depends(get_db)):
    return get_books_by_most_recent_published_year(db)

@router.get("/books/earliestPublishedYear", response_model=List[BookSchema], tags=["Books"])
def get_earliest_published_year_books_route(db: Session = Depends(get_db)):
    return get_books_by_earliest_published_year(db)

@router.get("/books/topRated", response_model=List[BookSchema], tags=["Books"])
def get_top_rated_books_route(db: Session = Depends(get_db)):
    return get_books_by_top_rated(db)

@router.get("/books/leastRated", response_model=List[BookSchema], tags=["Books"])
